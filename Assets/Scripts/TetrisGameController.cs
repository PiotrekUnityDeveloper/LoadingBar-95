...
public class TetrisGameController : MonoBehaviour {
    ...
    void InitGame() {
        ...
        gameModeValue.text = (controller.GetMode() == Mode.stage ? "S T A G E" : "C L A S S I C") + "  M O D E";
    ...
    public void ExitLoadtris()
    {
        SceneManager.LoadScene("LB95");
    }

    public void ButtonMove(string move)
    {
        ...
        else if (!isEndTurn && !gameOver && !gameClear && !isPaused && !isShowingAnimation)
        {
            ...
            else if (move == "right" && Time.time - previousToRight > 0.1f)
            {
                HorizontalMove(Vector3.right);
                previousToRight = Time.time;
            }
            else if (move == "left" && Time.time - previousToLeft > 0.1f)
            {
            ...
            else if (move == "down")
            {
                ...
                if (isAnimating && !isEndTurn)
                {
                    EndTurn();
                    isEndTurn = false;
                } // bugged (todo: fix that bro) // EDIT: i hope its fixed now :)
    ...
    public void OpenLoadtrisHelp()
    {
        
        helpObj.SetActive(true);
        StartCoroutine(cooldownHelpDialog());
       //Time.timeScale = 0f;
    }

    public IEnumerator cooldownHelpDialog()
    {
